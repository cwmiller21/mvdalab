plsFit <- function(formula, ncomp, data, subset, na.action, contr = "contr.none",
                   ...
                   x = FALSE, y = FALSE, ...)
{
  ...
  method <- match.arg(method, c("bidiagpls", "pls1gm"))  
  ...
  X2 <- delete.intercept(model.matrix(mt, mf))
  nobj <- dim(X)[1]
  npred <- dim(X)[2]
  ...
  fitFunc <- switch(method, bidiagpls = bidiagpls.fit, pls1gm = pls1gm.fit)
  if (sdscale == TRUE) {
