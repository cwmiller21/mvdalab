ellipse.mvdalab <- function(data, center = c(0, 0), radius = "chi", scale = TRUE, segments = 51, level = c(.95, .99), ...) {
...
output <- with(Ellipse[[1]], ggplot(Ellipse[[1]], aes(`Axis 1`, `Axis 2`)) + 
    ...
    geom_hline(yintercept = center[1], col = "lightgrey") + 
    geom_vline(xintercept = center[2], col = "lightgrey") + 
...
if(length(level) > 1) {
output <- output + sapply(2:length(Ellipse), function(x) geom_path(data = Ellipse[[x]], aes(`Axis 1`, `Axis 2`), col = x))
print(output)
} 
else print(output)
}