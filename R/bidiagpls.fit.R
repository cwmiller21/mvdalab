bidiagpls.fit <- function (X, Y, ncomp, ...) {
  ...
  if(A == 1) {
    W[, 1] <- crossprod(X, Y); W[, 1] <- W[, 1] / sqrt(c(crossprod(W[, 1]))) 
    TT[, 1] <- X %*% W[, 1]
    ...
    out <- NULL
    for(a in 2:A) {
      W[, a] <- crossprod(X, TT[, a - 1]) - D2[a - 1, a - 1] * W[, a - 1]
      W[, a] <- W[, a] - W[, 1:(a - 1)] %*% crossprod(W[, 1:(a - 1)], W[, a])
      D2[a - 1, a] <- -sqrt(c(crossprod(W[, a]))) 
      W[, a] <- W[, a] / D2[a - 1, a] 
      TT[, a] <- (X %*% W[, a]) - D2[a - 1, a] * TT[, a - 1]
      TT[, a] <- TT[, a] - TT[, 1:(a - 1)] %*% crossprod(TT[, 1:(a - 1)], TT[, a]) 
      D2[a, a] <- sqrt(c(crossprod(TT[, a]))) 
      TT[, a] <- TT[, a] / D2[a, a]
      iD2[a, a] <- 1 / D2[a, a]
      iD2[1:(a-1), a] <- iD2[1:(a - 1), a - 1] * (-D2[a - 1, a] / D2[a, a])
      q1 <- crossprod(TT[, a], Y)
      tQ[a, ] <- q1
